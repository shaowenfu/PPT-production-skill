...
- for `.docx`-derived pages that are primarily images, timelines, comparison charts, or infographic pages, do not treat them as unconstrained `image_only` pages. Extract and preserve the key visible text nodes (title, year labels, model names, axis labels, table headers, legend text, etc.), use them as source_text.
...
- make sure to maintain consistency between the user-provided copy and the final prompt text for locked pages. The user-provided copy is the source of truth and must be preserved in the final prompts without deletion or unapproved expansion. Don't split pages, do not add extra text or delete required text.
